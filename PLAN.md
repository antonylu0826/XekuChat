...
### Phase 8 — Integration API（類 LINE Messaging API）✅

**Part A — 資料模型 + API Key 管理：**
- [x] Schema migration：Integration（含 Bot User）、IntegrationChannel、IntegrationAuditLog、WebhookDelivery
- [x] Bot User 模式：每個 Integration 自動建立 `isBot: true` 的 User 作為發送者身份
- [x] API Key 產生：格式 `xk_live_<random32hex>`，建立時顯示一次
- [x] API Key 儲存：DB 只存 `apiKeyHash`（SHA-256）+ `apiKeyPrefix`（前 8 字元供辨識）
- [x] API Key 認證 middleware（獨立於 JWT，Bearer token → hash → 比對）
- [x] 速率限制：Redis 滑動視窗（per-integration，預設 60 req/min）
- [x] Idempotency Key：`X-Idempotency-Key` header → Redis 快取結果（TTL 24h），防止重複發送

**Part B — REST API 端點：**
- [x] `POST /api/v1/messages/push` — 發送訊息給指定使用者（自動建立或找到 DM 頻道）
- [x] `POST /api/v1/messages/broadcast` — 發送訊息至指定頻道
- [x] `POST /api/v1/messages/reply` — 回覆指定訊息（帶 `replyToMessageId`，含原始訊息內容）
- [x] `GET /api/v1/channels` — 取得組織頻道列表
- [x] `GET /api/v1/users` — 取得組織成員列表
- [x] `GET /api/v1/channels/:id/messages` — 取得頻道訊息（cursor 分頁）
- [x] 支援訊息類型：文字、圖片（URL）、檔案（URL）
- [x] API 呼叫稽核日誌（IntegrationAuditLog）

**Part C — Webhook 回呼：**
- [x] 事件觸發：`message.created`、`message.mention`（從 WS handler 發出）
- [x] HMAC-SHA256 簽名：`X-XekuChat-Signature: sha256=<hmac>`
- [x] 佇列制投遞：事件 → WebhookDelivery 表（pending）→ 背景 worker 發送
- [x] 失敗重試：最多 3 次，指數退避，30s 週期 worker
- [x] Admin 後台可查看投遞紀錄與狀態

**Part D — Admin 後台：**
- [x] Integrations 分頁 CRUD（名稱、Webhook URL、速率限制）
- [x] 建立後一次性顯示完整 API Key + Webhook Secret（之後只顯示前綴 `xk_live_a3f8...`）
- [x] 重新產生 Key、啟用/停用、刪除
- [x] 頻道權限設定（IntegrationChannel 綁定，未綁定 = 全組織存取）
- [x] API 呼叫紀錄 + Webhook 投遞計數

### Phase 9 — 維運強化（部分完成）

- [x] 語音 / 視訊通話（WebRTC P2P，DM 頻道，STUN NAT 穿透）
- [ ] 群組通話 SFU（mediasoup 自架，≥3人）
...
model Integration {
  ...
  description   String?
  apiKeyHash    String   @unique      // SHA-256 hash，驗證用
  apiKeyPrefix  String                // "xk_live_a3f8..." 供 UI 辨識
  webhookUrl    String?               // 事件回呼 URL
  webhookSecret String?               // HMAC-SHA256 簽名密鑰
  rateLimit     Int      @default(60) // 每分鐘請求上限
  isActive      Boolean  @default(true)
  botUserId     String   @unique
  botUser       User     @relation(fields: [botUserId], references: [id])
...
| Phase 8 | Integration API | 類 LINE Messaging API、Bot User、API Key hash、Webhook 佇列投遞、頻道權限範圍、Idempotency |
| Phase 9 | 維運強化（部分） | 語音/視訊通話（WebRTC P2P ✅）、群組通話 SFU、備份、Prometheus + Grafana 監控、訊息保留政策 |

---

*最後更新：2026-03-19（Phase 6 AI 聊天助理完成；Phase 7 AI 技能 + MCP 規劃完成；Phase 8 Integration API 完成；Phase 9 語音/視訊通話 WebRTC P2P 完成）*
