...
- [ ] Schema migration：Integration（含 Bot User）、IntegrationChannel、WebhookDelivery
- [ ] Bot User 模式：每個 Integration 自動建立 `isBot: true` 的 User 作為發送者身份
- [ ] API Key 產生：格式 `xk_live_<random32hex>`，建立時顯示一次
- [ ] API Key 儲存：DB 只存 `apiKeyHash`（SHA-256）+ `apiKeyPrefix`（前 8 字元供辨識）
- [ ] API Key 認證 middleware（獨立於 JWT，Bearer token → hash → 比對）
- [ ] 速率限制：Redis 滑動視窗（per-integration，預設 60 req/min）
- [ ] Idempotency Key：`X-Idempotency-Key` header → Redis 快取結果（TTL 24h），防止重複發送

**Part B — REST API 端點：**
- [ ] `POST /api/v1/messages/push` — 發送訊息給指定使用者（自動建立或找到 DM 頻道）
- [ ] `POST /api/v1/messages/broadcast` — 發送訊息至指定頻道
- [ ] `POST /api/v1/messages/reply` — 回覆指定訊息（帶 `replyToMessageId`）
- [ ] `GET /api/v1/channels` — 取得組織頻道列表
- [ ] `GET /api/v1/users` — 取得組織成員列表
- [ ] `GET /api/v1/channels/:id/messages` — 取得頻道訊息（分頁）
- [ ] 支援訊息類型：文字、圖片（URL）、檔案（URL）、Markdown
- [ ] API 呼叫稽核日誌（IntegrationAuditLog）

**Part C — Webhook 回呼：**
- [ ] 事件觸發：`message.created`、`message.mention`
- [ ] HMAC-SHA256 簽名：`X-XekuChat-Signature: sha256=<hmac>`
- [ ] 佇列制投遞：事件 → WebhookDelivery 表（pending）→ 背景 worker 發送
- [ ] 失敗重試：最多 3 次，指數退避（10s / 60s / 300s）
- [ ] Admin 後台可查看投遞紀錄與狀態

**Part D — Admin 後台：**
- [ ] Integrations 分頁 CRUD（名稱、Webhook URL、速率限制）
- [ ] 建立後一次性顯示完整 API Key（之後只顯示前綴 `xk_live_a3f8...`）
- [ ] 重新產生 Key、啟用/停用、刪除
- [ ] 頻道權限設定（IntegrationChannel 綁定，未綁定 = 全組織存取）
- [ ] API 呼叫紀錄 + Webhook 投遞紀錄

**實作順序：**
```
1. Schema migration（Integration + IntegrationChannel + WebhookDelivery + bot User）
2. API Key 產生 / hash / 驗證 middleware + rate limit（Redis 滑動視窗）
3. Admin 後台 CRUD（含一次性顯示 key、頻道權限設定）
4. REST 端點（push / broadcast / reply / 查詢）
5. Webhook 投遞佇列 + 背景 worker + HMAC 簽名
6. Admin 後台：投遞紀錄 + API 呼叫紀錄
```
...
| Phase 9 | 維運強化 | 通話（WebRTC）、備份、Prometheus + Grafana 監控、訊息保留政策 |

---

*最後更新：2026-03-17（Phase 6 完成；Phase 7 AI 技能 + MCP 規劃完成；Phase 8 Integration API 詳細規劃完成）*
